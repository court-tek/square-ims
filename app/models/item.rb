class Item
  ...
  class << self 
    def find(id)
      item = API.catalog.retrieve_catalog_object(
          object_id: id
      )
      raise KeyError, "no item found for this id `#{id}`" unless item.success?
    ...
    def all
      loop do 
        item_list = API.catalog.list_catalog
        if item_list.success?
          return item_list.data.objects
        elsif item_list.error?
          warn item_list.errors
        end
      end
    end
